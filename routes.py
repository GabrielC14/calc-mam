...
allowed_origins = [
    "https://spotify.gabrielvc.com.br",
    "https://calculoperfil.gabrielvc.com.br"
]
# Aplica a configuração de CORS apenas para as rotas de API
...
def login():
    ...
    auth_url = f"{AUTH_URL}?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&scope={scope}&show_dialog=true"
...
def now_playing():
    ...
    if not access_token: return jsonify({"error": "Auth required", "login_url": url_for('login')}), 401
...
def get_playlists():
    access_token = get_access_token()
    if not access_token:
        return jsonify({"error": "Auth required", "login_url": url_for('login')}), 401
